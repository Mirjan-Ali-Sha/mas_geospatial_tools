...
class MasGeospatialProvider(QgsProcessingProvider):
    ...
    def loadAlgorithms(self):
        ...
        def load_alg(module_path, class_name):
            try:
                import importlib
                module = importlib.import_module(module_path, package='mas_geospatial_tools')
                alg_class = getattr(module, class_name)
                self.addAlgorithm(alg_class())
            except Exception as e:
                QgsMessageLog.logMessage(f"Failed to load {class_name}: {str(e)}", "MAS Geospatial Tools", Qgis.Critical)
        ...
        # Hydrological
        load_alg('.algorithms.hydrological.flow_direction', 'D8FlowDirectionAlgorithm')
        load_alg('.algorithms.hydrological.flow_accumulation', 'D8FlowAccumulationAlgorithm')
    ...
    def name(self):
        ...
        return self.tr('MAS Geospatial Tools')
